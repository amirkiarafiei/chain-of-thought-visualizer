...
import { Settings, ChevronRight, ExternalLink, ShieldAlert, Key, Bot, X } from "lucide-react";
...
export default function SettingsSidebar({
  ...
  return (
    <>
      {/* Toggle Button - Fixed position at bottom-left, always visible */}
      <motion.button
        onClick={() => setIsExpanded(!isExpanded)}
        className={`fixed bottom-20 left-4 z-50 flex items-center gap-2 px-3 py-2.5 rounded-xl backdrop-blur-md transition-all shadow-lg border ${
          isExpanded 
            ? "bg-zinc-700 text-white border-white/20" 
            : "bg-zinc-800/90 hover:bg-zinc-700 text-blue-400 hover:text-blue-300 border-white/10"
        }`}
        ...
        {isExpanded && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 h-full w-[300px] bg-zinc-900/95 backdrop-blur-xl border-r border-white/10 z-40 shadow-2xl"
            >
              <div className="p-5 h-full flex flex-col gap-4 overflow-y-auto pt-16">
                {/* Header */}
